...
import { zeroArray, maxfn, assertArray, roundToPlace } from './utils';
...
export class Color {
  ...
  model: keyof typeof convert | undefined | null;
  valpha;

  constructor(object: ColorParam, model?: keyof typeof convert | null) {
    // class constructor 必须要有new关键字不可以直接调用构造函数
    ...
    if (object == null) {
      ...
      this.color = (object.color as number[]).slice();
  ...
  get chroma() {
    return getset('hcg', 1, maxfn(100));
  }

  get gray() {
    return getset('hcg', 2, maxfn(100));
  }
  ...
  get cyan() {
    return getset('cmyk', 0, maxfn(100));
  }

  get magenta() {
    return getset('cmyk', 1, maxfn(100));
  }

  get yellow() {
    return getset('cmyk', 2, maxfn(100));
  }

  get black() {
    return getset('cmyk', 3, maxfn(100));
  }

  get x() {
    return getset('xyz', 0, maxfn(100));
  }

  get y() {
    return getset('xyz', 1, maxfn(100));
  }

  get z() {
    return getset('xyz', 2, maxfn(100));
  }

  get l() {
    return getset('lab', 0, maxfn(100));
  }
  ...
  luminosity() {
    ...
    for (const [i, element] of rgb.entries()) {
      ...
      lum[i] =
        chan <= 0.039_28 ? chan / 12.92 : ((chan + 0.055) / 1.055) ** 2.4;
    }
...
function getset(model, channel, modifier?: any) {
