...
// 将navigator.userAgentData转换navigator.userAgent
 ...
 *
 */
async function getUserAgentUsingClientHints(hints: any) {
    // Helper functions for platform specific strings
    const GetCrosSpecificString = (values: { architecture: string; platformVersion: string; }) => {
        let newUA = 'X11; CrOS ';
    ...
    const GetWindowsSpecificString = (values: { platformVersion: string; architecture: string; }) => {
    ...
    const GetMacSpecificString = (values: { platformVersion: string; }) => {
    ...
    const Initialize = (values: { platform: string; platformVersion: string; architecture: string; uaFullVersion: string; model: string; }, fallback_version: string) => {
        if (!values.platform) {
 ...
 */
async function overrideUserAgentUsingClientHints(hints: any) {
